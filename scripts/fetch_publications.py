...
from config_helper import get_orcid_id

ORCID_ID = get_orcid_id()
ORCID_WORKS_URL = f"https://pub.orcid.org/v3.0/{ORCID_ID}/works"
OUTPUT_PATH = Path(__file__).resolve().parents[1] / "data" / "publications.json"
...
def main() -> None:
    # Load existing publications to preserve dates
    existing_dates_by_doi = {}
    existing_dates_by_title = {}
    if OUTPUT_PATH.exists():
        try:
            old_pubs = json.loads(OUTPUT_PATH.read_text(encoding="utf-8"))
            for p in old_pubs:
                d = p.get("date")
                if d:
                    if p.get("doi"):
                        existing_dates_by_doi[p["doi"].lower().strip()] = d
                    if p.get("title"):
                        existing_dates_by_title[p["title"].lower().strip()] = d
        except Exception:
            pass
    ...
    for group in groups:
        ...
        # Restore date if available in existing data
        doi_key = publication.get("doi", "").lower().strip()
        title_key = publication.get("title", "").lower().strip()
        if doi_key in existing_dates_by_doi:
            publication["date"] = existing_dates_by_doi[doi_key]
        elif title_key in existing_dates_by_title:
            publication["date"] = existing_dates_by_title[title_key]
