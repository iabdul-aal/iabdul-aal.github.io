...
SOURCES_PATH = PROJECT_ROOT / "data" / "talk_sources.json"
OUTPUT_PATH = PROJECT_ROOT / "data" / "talks.json"
...
YOUTUBE_DATE_PATTERNS = [
    re.compile(r'itemprop="datePublished" content="([0-9]{4}-[0-9]{2}-[0-9]{2})'),
    re.compile(r'"publishDate":"([0-9]{4}-[0-9]{2}-[0-9]{2})'),
    re.compile(r'"uploadDate":"([0-9]{4}-[0-9]{2}-[0-9]{2})'),
]
...
def fetch_featured_talk(url: str, event: str, format_name: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        for pattern in YOUTUBE_DATE_PATTERNS:
            date_match = pattern.search(video_page)
            if date_match:
                date = date_match.group(1)
                break
        if not date:
            # Fallback for newer YouTube watch page structures
            m = re.search(r'"publishDate":\s*\{\s*"simpleText"\s*:\s*"([^"]+)"\}', video_page)
            if m:
                raw_date = m.group(1)
